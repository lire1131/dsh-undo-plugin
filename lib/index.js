...
async function restore(cfg, mode, id) {
  ...
  if (mode === 'undo') {
    ...
    try {
      ...
      await appendRollbackLog(cfg, { mode: 'undo', targetId: target.s.id, targetKind: target.s.kind, preSnapshotId: pre.id, files: restored });
  ...
  if (mode === 'redo') {
    ...
    try {
      ...
      await appendRollbackLog(cfg, { mode: 'redo', targetId: pre.id, files: restored });
  ...
  try {
    ...
    const remounted = await ensureMount(cfg);
    await appendRollbackLog(cfg, { mode: 'restore', targetId: target.id, targetKind: target.kind, preSnapshotId: pre.id, files: restored });
