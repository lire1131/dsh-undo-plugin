...
async function restore(cfg, mode, id) {
  ...
  if (mode === 'undo') {
    ...
    const target = sameState(cur, newest.st) ? candidates[1] ?? null : newest;
    if (!target) return { ok: false, error: 'already at the earliest snapshot (nothing to undo)' };
    const stepped = target !== newest;
    ...
    if (stepped) await markFlag(cfg, newest.s, 'stepped', true);
...
function renderRestoreResult(r) {
  ...
  if (r.stepped) lines.push('(stepped back past the post-change record)');
...
export function apply(ctx, config = {}) {
  ...
  if (config.watch !== false) {
    ...
    const onEvent = (_event, filename) => {
      if (typeof filename === 'string' && WATCHED_BASENAMES.has(basename(filename))) schedule();
    };
    ctx.effect(() => {
      const watchers = [];
      ...
      handler: async (req, res) => {
        try {
          ...
          if (method === 'GET' && path === '/api/undo/status') {
            ...
            const cur = await currentState(cfg);
            const candidates = await undoCandidates(cfg, list);
            const canUndo = candidates.length >= 2 || (candidates.length === 1 && !sameState(cur, candidates[0].st));
            const pre = list.find((s) => s.kind === 'pre-restore' && !s.consumed);
