...
export function apply(ctx, config = {}) {
  ...
  void (async () => {
    try {
      ...
      ctx.logger.info(`[dsh-undo] baseline snapshot ${snap.id}${prunedN > 0 ? ` (pruned ${prunedN})` : ''}`);
    } catch (error) {
    ...
    const schedule = () => {
      ...
      timer = setTimeout(async () => {
        ...
        try {
          ...
          const pruned = await pruneAuto(cfg, list);
          const prunedN = pruned.removedAuto + pruned.removedPre;
          if (snap.files.length > 0) ctx.logger.info(`[dsh-undo] auto snapshot ${snap.id} (${snap.files.length} file(s), ${reason}${prunedN > 0 ? `, pruned ${prunedN}` : ''})`);
        } catch (error) {
