...
/**
 * 快照内敏感文件的"对比内容"：本机 vault 有真实值用真实值（diff 显示真实差异），
 * 否则用快照里的脱敏版。返回 Buffer 或 null。
 */
async function snapSensitiveBuf(cfg, snap, destName) {
  const ref = snap.envVaultRefs?.[destName];
  if (ref) {
