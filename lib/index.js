 ...
 * DSH home directory: prefer DSH_HOME env (issue #6 — third-party clients with
 * non-default home), fall back to ~/.dsh. All .dsh-relative paths (settings,
 * snapshots, profiles) are based on DSH_HOME_DIR; node_modules discovery stays
 * on HOME (user-level, not inside .dsh).
 */
...
const DSH_HOME_DIR = (process.env.DSH_HOME && process.env.DSH_HOME.trim().length > 0)
...
/** Legacy flat snapshot root (kept for migration/back-compat). Issue #6: use DSH_HOME_DIR. */
const LEGACY_ROOT = process.env.DSH_UNDO_ROOT ?? join(DSH_HOME_DIR, 'undo-snapshots');
const SETTINGS_FILE = process.env.DSH_UNDO_SETTINGS ?? join(DSH_HOME_DIR, 'undo', 'settings.json');
...
function rootDir(cfg, root) {
  return root === 'profile'
    ? (cfg.profileDir ?? join(DSH_HOME_DIR, 'profiles', 'web'))
    : (cfg.homeDir ?? DSH_HOME_DIR);
}
...
async function safeModeSet(cfg, on) {
  ...
  if (on) {
    ...
    else await fs.writeFile(backup, '# (no cordis.patch.yml existed when safe mode was entered)\n[]\n', 'utf8');
...
export function apply(ctx, config = {}) {
  ...
  const cfg = {
    ...
    profileDir: config.profileDir ?? join(DSH_HOME_DIR, 'profiles', profileName),
