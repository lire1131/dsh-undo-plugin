window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var styles = {
			...
			row: "u_row", pair: "u_pair", keyLabel: "u_keyLabel", keyInput: "u_keyInput", num: "u_num", dir: "u_dir", hint: "u_hint",
			save: "u_save", close: "u_close"
		};
		//#endregion
		//#region locales
		const NS = "undo";
		...
		const en = {
			...
			"snapshots.aria": "Open snapshot manager",
			"ok.undo": "Undone → {id}",
			"ok.redo": "Redone",
		...
		function UndoHeader({ t, onOpenPanel }) {
			...
			const run = async (label, path) => {
				...
				try {
					...
					let text;
					if (r && r.ok) {
						text = r.message || (label === "undo" ? t("ok.undo", { id: r.targetId || "" }) : t("ok.redo"));
					} else {
		...
		function SnapshotPanel({ onClose, t }) {
			...
			const restoreOne = (s) => {
				if (!window.confirm(t("panel.confirm.restore"))) return;
				act("undo", "/api/undo/restore", { id: s.id });
			};
			const deleteOne = (s) => {
		...
		//#endregion
		//#region SnapshotSettingsRow (settings.general.item)
		/** Collapsible snapshot settings row: auto-save toggle, timings, keep count, store dirs. */
		function SnapshotSettingsRow({ t }) {
			...
			return (0, react_jsx_runtime.jsx)("div", {
				className: styles.row,
				"data-undo-settings": true,
				children: [
					(0, react_jsx_runtime.jsx)("button", {
						type: "button", className: styles.save,
						onClick: () => setOpen(!open),
						children: (open ? "▾ " : "▸ ") + t("settings.title")
					}),
					open && s !== null && (0, react_jsx_runtime.jsxs)(react.Fragment, {
						children: [
							(0, react_jsx_runtime.jsx)("div", { className: styles.pair, children: [
								(0, react_jsx_runtime.jsx)("label", { className: styles.keyLabel, children: (0, react_jsx_runtime.jsx)("input", { type: "checkbox", checked: !!s.autoEnabled, onChange: (e) => set("autoEnabled", e.target.checked) }) + " " + t("settings.auto") })
							] }),
							(0, react_jsx_runtime.jsx)("div", { className: styles.pair, children: [
		...
		function apply(ctx) {
			...
			// Snapshot settings row (auto-save params, store dirs)
			ctx.slots.inject("settings.general.item", () => ctx.slots.register({
				name: "settings.general.item",
				id: "undo-settings",
				order: 40,
			...
			ctx.effect(() => {
				const onKeyDown = (e) => {
					...
					if (keys.undo && keyEventMatches(e, keys.undo)) {
						e.preventDefault();
						api("/api/undo/undo", "POST").then((r) => publishResult(r, "undo"));
						return;
					}
					if (keys.redo && keyEventMatches(e, keys.redo)) {
						e.preventDefault();
						api("/api/undo/redo", "POST").then((r) => publishResult(r, "redo"));
					}
				};
		...
		exports.SnapshotSettingsRow = SnapshotSettingsRow;
		exports.apply = apply;
