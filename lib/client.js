window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css2 = ".u_icon{display:inline-block;vertical-align:-2px;flex:none;line-height:0}.u_badge{cursor:pointer;border:1px solid var(--dsw-alias-border-l1, rgba(128,128,128,.35));background:var(--dsw-specific-tip, transparent);color:var(--dsw-alias-label-secondary, inherit);border-radius:999px;height:20px;padding:0 8px;font-size:11px;line-height:18px;display:inline-flex;align-items:center;gap:5px;white-space:nowrap;flex:none}.u_badge:hover{background:var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.15));color:var(--dsw-alias-label-primary, inherit)}.u_dot{width:6px;height:6px;border-radius:50%;background:var(--dsw-alias-state-success-primary, #30a46c);flex:none}.u_subtitle{font-size:11px;color:var(--dsw-alias-label-tertiary, #888);font-weight:400;white-space:nowrap;max-width:min(220px,30vw);text-overflow:ellipsis;overflow:hidden}";
		...
		function apply(ctx) {
			...
			safe("slots:settings.general.item", () => ctx.slots.inject("settings.general.item", () => ctx.slots.register({
